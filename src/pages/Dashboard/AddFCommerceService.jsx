...
const AddFCommerceService = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire("Success!", "Digital fCommerce Service Data added successfully!", "success");
        setImageUrl(""); // Reset input field after success
      } else {
        Swal.fire("Error", data.message || "Failed to add fCommerce Service Data.", "error");
      }
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Failed to add fCommerce Service Data.", "error");
    }
  };

  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md text-black">
      <h2 className="text-xl font-semibold mb-4 text-purple-600 text-center">Add F-Commerce Service (Image URL)</h2>
