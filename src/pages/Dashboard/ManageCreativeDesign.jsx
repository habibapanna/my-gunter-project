...
const ManageCreativeDesign = () => {
  ...
  const fetchDesigns = async () => {
    try {
      const res = await axios.get("http://localhost:5000/creative-designs");
      setDesigns(res.data);
    } catch (error) {
  ...
  const handleEdit = async (id, currentImageUrl) => {
    ...
    if (imageUrl) {
      try {
        await axios.put(`http://localhost:5000/creative-designs/${id}`, { imageUrl });
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:5000/creative-designs/${id}`);
  ...
  return (
    <div className="p-4 overflow-x-auto">
      <h2 className="text-xl lg:text-2xl font-bold mb-4 text-orange-600 text-center">Manage Creative Design</h2>
