...
const AddWebDevelopment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://my-gunter-project-server.vercel.app/web-development", {
        method: "POST",
      ...
      if (res.ok) {
        Swal.fire("Success!", "Development added successfully!", "success");
        setImageUrl(""); // Reset input field after success
      } else {
        Swal.fire("Error", data.message || "Failed to add development.", "error");
      }
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Failed to add development.", "error");
    }
  };

  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md text-black">
      <h2 className="text-xl font-semibold mb-4 text-orange-600 text-center">Add Web Development (Image URL)</h2>
